...
public class TransitionBuilder {
    ...
    private static final String f3185a = "TransitionBuilder";

    private static void a(MotionScene scene, MotionScene.Transition transition, ConstraintSet startConstraintSet, ConstraintSet endConstraintSet) {
    ...
    public static MotionScene.Transition buildTransition(MotionScene scene, int transitionId, int startConstraintSetId, ConstraintSet startConstraintSet, int endConstraintSetId, ConstraintSet endConstraintSet) {
        ...
        a(scene, transition, startConstraintSet, endConstraintSet);
        return transition;
    }

    public static void validate(MotionLayout layout) {
        MotionScene motionScene = layout.G;
        if (motionScene != null) {
            if (motionScene.validateLayout(layout)) {
                if (motionScene.f3094c == null || motionScene.getDefinedTransitions().isEmpty()) {
