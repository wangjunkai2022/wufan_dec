...
public final class DiskLruCache implements Closeable, Flushable {
    ...
    public final class Entry {
        ...
        Entry(String str) {
            ...
            for (int i4 = 0; i4 < DiskLruCache.this.valueCount; i4++) {
                sb.append(i4);
                this.cleanFiles[i4] = new File(DiskLruCache.this.directory, sb.toString());
                sb.append(".tmp");
                this.dirtyFiles[i4] = new File(DiskLruCache.this.directory, sb.toString());
                sb.setLength(length);
            }
        }
                ...
                while (true) {
                    try {
                        ...
                        if (i4 < diskLruCache.valueCount) {
                            sourceArr[i4] = diskLruCache.fileSystem.source(this.cleanFiles[i4]);
                            i4++;
                        } else {
    ...
    DiskLruCache(FileSystem fileSystem, File file, int i2, int i4, long j4, Executor executor) {
    ...
    public static DiskLruCache create(FileSystem fileSystem, File file, int i2, int i4, long j4) {
        if (j4 > 0) {
            if (i4 > 0) {
                return new DiskLruCache(fileSystem, file, i2, i4, j4, new ThreadPoolExecutor(0, 1, 60L, TimeUnit.SECONDS, new LinkedBlockingQueue(), Util.threadFactory("OkHttp DiskLruCache", true)));
            }
    ...
    private void validateKey(String str) {
        ...
        throw new IllegalArgumentException("keys must match regex [a-z0-9_-]{1,120}: \"" + str + a.f72569g);
    }
    ...
    synchronized void completeEdit(Editor editor, boolean z3) throws IOException {
        ...
        if (entry.currentEditor == editor) {
            ...
            for (int i4 = 0; i4 < this.valueCount; i4++) {
                File file = entry.dirtyFiles[i4];
                if (z3) {
                    if (this.fileSystem.exists(file)) {
                        File file2 = entry.cleanFiles[i4];
                        this.fileSystem.rename(file, file2);
                        long j4 = entry.lengths[i4];
                        long size = this.fileSystem.size(file2);
                        entry.lengths[i4] = size;
