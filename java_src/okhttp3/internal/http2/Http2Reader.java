...
public final class Http2Reader implements Closeable {
        ...
        public long read(Buffer buffer, long j4) throws IOException {
            while (true) {
                ...
                if (i2 == 0) {
                    ...
                    long read = this.source.read(buffer, Math.min(j4, i2));
                    if (read == -1) {
                        return -1L;
                    }
    ...
    public interface Handler {
        ...
        void alternateService(int i2, String str, ByteString byteString, String str2, int i4, long j4);

        void data(boolean z3, int i2, BufferedSource bufferedSource, int i4) throws IOException;

        void goAway(int i2, ErrorCode errorCode, ByteString byteString);

        void headers(boolean z3, int i2, int i4, List<Header> list);

        void ping(boolean z3, int i2, int i4);

        void priority(int i2, int i4, int i5, boolean z3);

        void pushPromise(int i2, int i4, List<Header> list) throws IOException;

        void rstStream(int i2, ErrorCode errorCode);

        void settings(boolean z3, Settings settings);

        void windowUpdate(int i2, long j4);
    }
    ...
    static int lengthWithoutPadding(int i2, byte b4, short s3) throws IOException {
        if ((b4 & 8) != 0) {
            i2--;
        }
        if (s3 <= i2) {
        ...
        throw Http2.ioException("PROTOCOL_ERROR padding %s > remaining length %s", Short.valueOf(s3), Integer.valueOf(i2));
    }

    private void readData(Handler handler, int i2, byte b4, int i4) throws IOException {
        if (i4 == 0) {
        ...
        if (!((b4 & 32) != 0)) {
            ...
            handler.data(z3, i4, this.source, lengthWithoutPadding(i2, b4, readByte));
    ...
    private void readGoAway(Handler handler, int i2, byte b4, int i4) throws IOException {
        if (i2 < 8) {
    ...
    private List<Header> readHeaderBlock(int i2, short s3, byte b4, int i4) throws IOException {
    ...
    private void readHeaders(Handler handler, int i2, byte b4, int i4) throws IOException {
        if (i4 != 0) {
            boolean z3 = (b4 & 1) != 0;
            short readByte = (b4 & 8) != 0 ? (short) (this.source.readByte() & 255) : (short) 0;
            if ((b4 & 32) != 0) {
            ...
            handler.headers(z3, i4, -1, readHeaderBlock(lengthWithoutPadding(i2, b4, readByte), readByte, b4, i4));
            return;
        }
    ...
    private void readPing(Handler handler, int i2, byte b4, int i4) throws IOException {
        if (i2 != 8) {
    ...
    private void readPriority(Handler handler, int i2, byte b4, int i4) throws IOException {
        if (i2 != 5) {
    ...
    private void readPushPromise(Handler handler, int i2, byte b4, int i4) throws IOException {
        if (i4 != 0) {
            short readByte = (b4 & 8) != 0 ? (short) (this.source.readByte() & 255) : (short) 0;
            handler.pushPromise(i4, this.source.readInt() & Integer.MAX_VALUE, readHeaderBlock(lengthWithoutPadding(i2 - 4, b4, readByte), readByte, b4, i4));
            return;
        }
        throw Http2.ioException("PROTOCOL_ERROR: TYPE_PUSH_PROMISE streamId == 0", new Object[0]);
    }

    private void readRstStream(Handler handler, int i2, byte b4, int i4) throws IOException {
        if (i2 != 4) {
    ...
    private void readSettings(Handler handler, int i2, byte b4, int i4) throws IOException {
        if (i4 != 0) {
    ...
    private void readWindowUpdate(Handler handler, int i2, byte b4, int i4) throws IOException {
        if (i2 != 4) {
