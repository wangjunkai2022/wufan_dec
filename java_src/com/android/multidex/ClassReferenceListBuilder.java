...
public class ClassReferenceListBuilder {
    ...
    private void addClassWithHierachy(String str) {
        ...
        try {
            ...
            for (int i2 = 0; i2 < size; i2++) {
                addClassWithHierachy(interfaces.getType(i2).getClassName());
            }
    ...
    private void addDependencies(DirectClassFile directClassFile) {
        ...
        for (int i2 = 0; i2 < size; i2++) {
            checkDescriptor(fields.get(i2).getDescriptor().getString());
        }
        MethodList methods = directClassFile.getMethods();
        int size2 = methods.size();
        for (int i4 = 0; i4 < size2; i4++) {
            checkPrototype(Prototype.intern(methods.get(i4).getDescriptor().getString()));
        }
    }
    ...
    private void checkPrototype(Prototype prototype) {
        ...
        for (int i2 = 0; i2 < parameterTypes.size(); i2++) {
            checkDescriptor(parameterTypes.get(i2).getDescriptor());
        }
    }
