...
public class MainDexListBuilder {
    ...
    public MainDexListBuilder(boolean z3, String str, String str2) throws IOException {
        ...
        try {
            try {
                ...
                throw new IOException(a.f72569g + str + "\" can not be read as a zip archive. (" + e4.getMessage() + ")", e4);
            }
        } catch (Throwable th3) {
            th = th3;
            path = null;
            ...
            for (String str : classPathElement.list()) {
                if (str.endsWith(CLASS_EXTENSION)) {
                    ...
                    if (hasRuntimeVisibleAnnotation(directClassFile)) {
                        ...
                        while (true) {
                            if (i4 < methods.size()) {
                                if (hasRuntimeVisibleAnnotation(methods.get(i4))) {
                                    this.filesToKeep.add(str);
                                    break;
                                }
                                i4++;
    ...
    public static void main(String[] strArr) {
        ...
        while (i2 < strArr.length - 2) {
            if (strArr[i2].equals(DISABLE_ANNOTATION_RESOLUTION_WORKAROUND)) {
                z3 = false;
            } else {
                PrintStream printStream = System.err;
                printStream.println("Invalid option " + strArr[i2]);
                printUsage();
                System.exit(1);
            }
            i2++;
        ...
        try {
            printList(new MainDexListBuilder(z3, strArr[i2], strArr[i2 + 1]).getMainDexList());
        } catch (IOException e4) {
