...
public class EscapeAnalysis {
    ...
    public static class EscapeSet {
        ...
        EscapeSet(int i2, int i4, EscapeState escapeState) {
    ...
    private int findSetIndex(RegisterSpec registerSpec) {
        int i2 = 0;
        while (i2 < this.latticeValues.size() && !this.latticeValues.get(i2).regSet.get(registerSpec.getReg())) {
            i2++;
        }
        return i2;
    }
    ...
    private void insertPlainInsnBefore(SsaInsn ssaInsn, RegisterSpecList registerSpecList, RegisterSpec registerSpec, int i2, Constant constant) {
        Rop ropFor;
        Insn plainCstInsn;
        ...
        if (i2 == 56) {
            ...
            ropFor = Rops.ropFor(i2, registerSpec, registerSpecList, constant);
        }
        Rop rop = ropFor;
        if (constant == null) {
    ...
    private void insertThrowingInsnBefore(SsaInsn ssaInsn, RegisterSpecList registerSpecList, RegisterSpec registerSpec, int i2, Constant constant) {
        Insn throwingCstInsn;
        Insn originalRopInsn = ssaInsn.getOriginalRopInsn();
        Rop ropFor = Rops.ropFor(i2, registerSpec, registerSpecList, constant);
        if (constant == null) {
    ...
    private void movePropagate() {
        for (int i2 = 0; i2 < this.ssaMeth.getRegCount(); i2++) {
    ...
    private void replaceDef(SsaInsn ssaInsn, SsaInsn ssaInsn2, int i2, ArrayList<RegisterSpec> arrayList) {
    ...
    private void replaceUse(SsaInsn ssaInsn, SsaInsn ssaInsn2, ArrayList<RegisterSpec> arrayList, HashSet<SsaInsn> hashSet) {
        ...
        if (opcode == 34) {
            ...
            for (int i2 = 0; i2 < size; i2++) {
                RegisterSpec make = RegisterSpec.make(arrayList.get(i2).getReg(), (TypeBearer) initValues.get(i2));
                insertPlainInsnBefore(ssaInsn, RegisterSpecList.EMPTY, make, 5, initValues.get(i2));
                arrayList.set(i2, make);
            }
        } else if (opcode == 38) {
