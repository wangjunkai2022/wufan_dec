...
public final class FindUsages {
    ...
    private Set<Integer> findAssignableTypes(Dex dex, int i2) {
        ...
        for (ClassDef classDef : dex.classDefs()) {
            if (hashSet.contains(Integer.valueOf(classDef.getSupertypeIndex()))) {
                ...
                while (true) {
                    if (i4 >= length) {
                        break;
                    } else if (hashSet.contains(Integer.valueOf(interfaces[i4]))) {
                        hashSet.add(Integer.valueOf(classDef.getTypeIndex()));
                        break;
                    } else {
                        i4++;
    ...
    private Set<Integer> getFieldIds(Dex dex, Set<Integer> set, int i2) {
        HashSet hashSet = new HashSet();
        int i4 = 0;
        for (FieldId fieldId : dex.fieldIds()) {
            if (set.contains(Integer.valueOf(fieldId.getNameIndex())) && i2 == fieldId.getDeclaringClassIndex()) {
    ...
    private Set<Integer> getMethodIds(Dex dex, Set<Integer> set, int i2) {
        Set<Integer> findAssignableTypes = findAssignableTypes(dex, i2);
        HashSet hashSet = new HashSet();
        int i4 = 0;
    ...
    public String location() {
        ...
        if (this.currentMethod != null) {
            return str + g0.f27568a + this.dex.strings().get(this.dex.methodIds().get(this.currentMethod.getMethodIndex()).getNameIndex());
        }
        return str;
    }
