...
public final class RopTranslator {
        ...
        public void visitSwitchInsn(SwitchInsn switchInsn) {
            ...
            if (size == size2 - 1 && primarySuccessor == successors.get(size)) {
                ...
                for (int i2 = 0; i2 < size; i2++) {
                    codeAddressArr[i2] = RopTranslator.this.addresses.getStart(successors.get(i2));
                }
                CodeAddress codeAddress = new CodeAddress(position);
    ...
    private RopTranslator(RopMethod ropMethod, int i2, LocalVariableInfo localVariableInfo, int i4, DexOptions dexOptions) {
        ...
        this.paramSize = i4;
        boolean calculateParamsAreInOrder = calculateParamsAreInOrder(ropMethod, i4);
        this.paramsAreInOrder = calculateParamsAreInOrder;
        BasicBlockList blocks = ropMethod.getBlocks();
        int size = blocks.size();
        int i5 = size * 3;
        int instructionCount = blocks.getInstructionCount() + i5;
        int assignmentCount = localVariableInfo != null ? instructionCount + size + localVariableInfo.getAssignmentCount() : instructionCount;
        int regCount = blocks.getRegCount() + (calculateParamsAreInOrder ? 0 : i4);
        this.regCount = regCount;
        OutputCollector outputCollector = new OutputCollector(dexOptions, assignmentCount, i5, regCount, i4);
        this.output = outputCollector;
    ...
    private static boolean calculateParamsAreInOrder(RopMethod ropMethod, final int i2) {
        ...
        ropMethod.getBlocks().forEachInsn(new Insn.BaseVisitor() { // from class: com.android.dx.dex.code.RopTranslator.1
            ...
            public void visitPlainCstInsn(PlainCstInsn plainCstInsn) {
                if (plainCstInsn.getOpcode().getOpcode() == 3) {
                    ...
                    boolean[] zArr2 = zArr;
                    zArr2[0] = zArr2[0] && (regCount - i2) + value == plainCstInsn.getResult().getReg();
                }
            }
        });
        return zArr[0];
    ...
    private void outputBlock(BasicBlock basicBlock, int i2) {
        ...
        if (lastInsn.getOpcode().getBranchingness() == 4 && basicBlock.getSecondarySuccessor() == i2) {
    ...
    private void outputInstructions() {
        ...
        while (i2 < length) {
            int i4 = i2 + 1;
            outputBlock(blocks.labelToBlock(iArr[i2]), i4 == iArr.length ? -1 : iArr[i4]);
            i2 = i4;
        }
    }
    ...
    private void pickOrder() {
        ...
        for (int i4 = 0; i4 < size; i4++) {
            Bits.set(makeBitSet, blocks.get(i4).getLabel());
        }
        ...
        while (firstLabel != -1) {
            while (true) {
                ...
                for (int i6 = 0; i6 < size2; i6++) {
                    i2 = labelToPredecessors.get(i6);
                    if (Bits.get(makeBitSet2, i2)) {
                        break;
                    } else if (Bits.get(makeBitSet, i2) && blocks.labelToBlock(i2).getPrimarySuccessor() == firstLabel) {
                        break;
                    }
                }
    ...
    public static DalvCode translate(RopMethod ropMethod, int i2, LocalVariableInfo localVariableInfo, int i4, DexOptions dexOptions) {
        return new RopTranslator(ropMethod, i2, localVariableInfo, i4, dexOptions).translateAndGetResult();
    }
