...
public final class LocalList extends FixedSizeList {
    ...
    public static class MakeState {
        ...
        public MakeState(int i2) {
            this.result = new ArrayList<>(i2);
        }

        private void aboutToProcess(int i2, int i4) {
            int[] iArr = this.endIndices;
            boolean z3 = iArr == null;
            int i5 = this.lastAddress;
            if (i2 != i5 || z3) {
                if (i2 >= i5) {
                    if (z3 || i4 >= iArr.length) {
                        int i6 = i4 + 1;
        ...
        private void add(int i2, Disposition disposition, RegisterSpec registerSpec) {
            int reg = registerSpec.getReg();
            this.result.add(new Entry(i2, disposition, registerSpec));
        ...
        private void addOrUpdateEnd(int i2, Disposition disposition, RegisterSpec registerSpec) {
            if (disposition != Disposition.START) {
                int i4 = this.endIndices[registerSpec.getReg()];
                if (i4 >= 0) {
                    Entry entry = this.result.get(i4);
                    if (entry.getAddress() == i2 && entry.getRegisterSpec().equals(registerSpec)) {
                        this.result.set(i4, entry.withDisposition(disposition));
                        this.regs.remove(registerSpec);
                        return;
                    }
                }
                endLocal(i2, registerSpec, disposition);
                return;
            }
            throw new RuntimeException("shouldn't happen");
        }
        ...
        */
        ...
        public void endLocal(int i2, RegisterSpec registerSpec) {
            endLocal(i2, registerSpec, Disposition.END_SIMPLY);
        }
        ...
        public LocalList finish() {
            ...
            for (int i5 = 0; i5 < i2; i5++) {
                localList.set(i5, entryArr[i5]);
            }
            localList.setImmutable();
            return localList;
        }

        public void snapshot(int i2, RegisterSpecSet registerSpecSet) {
            int maxSize = registerSpecSet.getMaxSize();
            aboutToProcess(i2, maxSize - 1);
            for (int i4 = 0; i4 < maxSize; i4++) {
                RegisterSpec registerSpec = this.regs.get(i4);
                RegisterSpec filterSpec = filterSpec(registerSpecSet.get(i4));
        ...
        public void startLocal(int i2, RegisterSpec registerSpec) {
            ...
            int i4 = this.endIndices[reg];
            if (registerSpec4 != null) {
                add(i2, Disposition.END_REPLACED, registerSpec4);
            } else if (i4 >= 0) {
                Entry entry = this.result.get(i4);
                if (entry.getAddress() == i2) {
                    ...
                    this.result.set(i4, entry.withDisposition(Disposition.END_REPLACED));
                }
            }
            if (reg > 0 && (registerSpec3 = this.regs.get(reg - 1)) != null && registerSpec3.isCategory2()) {
                addOrUpdateEnd(i2, Disposition.END_CLOBBERED_BY_NEXT, registerSpec3);
            }
            if (filterSpec.isCategory2() && (registerSpec2 = this.regs.get(reg + 1)) != null) {
                addOrUpdateEnd(i2, Disposition.END_CLOBBERED_BY_PREV, registerSpec2);
            }
            add(i2, Disposition.START, filterSpec);
        }

        public void endLocal(int i2, RegisterSpec registerSpec, Disposition disposition) {
            int reg = registerSpec.getReg();
            RegisterSpec filterSpec = filterSpec(registerSpec);
            aboutToProcess(i2, reg);
            if (this.endIndices[reg] < 0 && !checkForEmptyRange(i2, filterSpec)) {
                add(i2, disposition, filterSpec);
            }
        }
    }
    ...
    private static void debugVerify(LocalList localList) {
        try {
            ...
            for (int i2 = 0; i2 < size; i2++) {
                System.err.println(localList.get(i2));
            }
            throw e4;
        }
    }
    ...
    public static LocalList make(DalvInsnList dalvInsnList) {
        ...
        for (int i2 = 0; i2 < size; i2++) {
            DalvInsn dalvInsn = dalvInsnList.get(i2);
