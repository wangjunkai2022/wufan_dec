...
public class DirectClassFile implements ClassFile {
    ...
    public static class DcfTypeList implements TypeList {
        ...
        public DcfTypeList(ByteArray byteArray, int i2, int i4, StdConstantPool stdConstantPool, ParseObserver parseObserver) {
            if (i4 >= 0) {
                ByteArray slice = byteArray.slice(i2, (i4 * 2) + i2);
                this.bytes = slice;
                this.size = i4;
                ...
                for (int i5 = 0; i5 < i4; i5++) {
                    int i6 = i5 * 2;
                    try {
                        CstType cstType = (CstType) stdConstantPool.get(slice.getUnsignedShort(i6));
                        if (parseObserver != null) {
                            parseObserver.parsed(slice, i6, 2, "  " + cstType);
                        }
        ...
        public Type getType(int i2) {
            return ((CstType) this.pool.get(this.bytes.getUnsignedShort(i2 * 2))).getClassType();
        }
    ...
    private boolean isGoodMagic(int i2) {
        return i2 == CLASS_FILE_MAGIC;
    }

    private boolean isGoodVersion(int i2, int i4) {
        if (i2 >= 0) {
            return i4 == 53 ? i2 <= 0 : i4 < 53 && i4 >= 45;
        }
        return false;
    }
    ...
    private void parse0() {
        if (this.bytes.size() >= 10) {
            ...
            if (this.strictParse) {
                if (isGoodMagic(getMagic0())) {
                    if (!isGoodVersion(getMinorVersion0(), getMajorVersion0())) {
                        throw new ParseException("unsupported class file version " + getMajorVersion0() + g0.f27568a + getMinorVersion0());
                    }
                } else {
            ...
            int i2 = endOffset + 2;
            this.thisClass = (CstType) this.pool.get(this.bytes.getUnsignedShort(i2));
            int i4 = endOffset + 4;
            this.superClass = (CstType) this.pool.get0Ok(this.bytes.getUnsignedShort(i4));
            int i5 = endOffset + 6;
            int unsignedShort2 = this.bytes.getUnsignedShort(i5);
            ...
            if (parseObserver5 != null) {
                ...
                parseObserver6.parsed(byteArray5, i2, 2, "this_class: " + this.thisClass);
                ParseObserver parseObserver7 = this.observer;
                ByteArray byteArray6 = this.bytes;
                parseObserver7.parsed(byteArray6, i4, 2, "super_class: " + stringOrNone(this.superClass));
                ParseObserver parseObserver8 = this.observer;
                ByteArray byteArray7 = this.bytes;
                parseObserver8.parsed(byteArray7, i5, 2, "interfaces_count: " + Hex.u2(unsignedShort2));
                if (unsignedShort2 != 0) {
                    this.observer.parsed(this.bytes, endOffset + 8, 0, "interfaces:");
                }
            }
            int i6 = endOffset + 8;
            this.interfaces = makeTypeList(i6, unsignedShort2);
            int i7 = i6 + (unsignedShort2 * 2);
            if (this.strictParse) {
            ...
            FieldListParser fieldListParser = new FieldListParser(this, this.thisClass, i7, this.attributeFactory);
    ...
    public TypeList makeTypeList(int i2, int i4) {
        if (i4 == 0) {
            return StdTypeList.EMPTY;
        }
        StdConstantPool stdConstantPool = this.pool;
        if (stdConstantPool != null) {
            return new DcfTypeList(this.bytes, i2, i4, stdConstantPool, this.observer);
        }
