    ...
    public static OneLocalsArray mergeLocals(OneLocalsArray oneLocalsArray, OneLocalsArray oneLocalsArray2) {
        ...
        if (oneLocalsArray2.getMaxLocals() == maxLocals) {
            for (int i2 = 0; i2 < maxLocals; i2++) {
                TypeBearer orNull = oneLocalsArray.getOrNull(i2);
                TypeBearer mergeType = mergeType(orNull, oneLocalsArray2.getOrNull(i2));
                if (mergeType != orNull) {
                    if (oneLocalsArray3 == null) {
                        oneLocalsArray3 = oneLocalsArray.copy();
                    }
                    if (mergeType == null) {
                        oneLocalsArray3.invalidate(i2);
                    } else {
                        oneLocalsArray3.set(i2, mergeType);
                    }
                }
            }
            if (oneLocalsArray3 == null) {
    ...
    public static ExecutionStack mergeStack(ExecutionStack executionStack, ExecutionStack executionStack2) {
        ...
        if (executionStack2.size() == size) {
            for (int i2 = 0; i2 < size; i2++) {
                TypeBearer peek = executionStack.peek(i2);
                TypeBearer peek2 = executionStack2.peek(i2);
                ...
                if (mergeType != peek) {
                    ...
                    if (mergeType != null) {
                        try {
                            executionStack3.change(i2, mergeType);
                        } catch (SimException e4) {
                            e4.addContext("...while merging stack[" + Hex.u2(i2) + "]");
                            throw e4;
                        }
                    } else {
