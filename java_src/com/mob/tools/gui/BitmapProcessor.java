...
public class BitmapProcessor {
    ...
    private static class PatchInputStream extends FilterInputStream {
        ...
        public long skip(long j4) throws IOException {
            long j5 = 0;
            while (j5 < j4) {
                long skip = this.f53851in.skip(j4 - j5);
                if (skip == 0) {
                    break;
                }
                j5 += skip;
    ...
    public static void deleteCacheDir(boolean z3) {
        if (z3) {
            deleteCacheDir();
        } else {
    ...
    public static String getCacheSizeText() {
        ...
        float f4 = cacheSizeInByte / 1024.0f;
        if (f4 < 1000.0f) {
            return String.format(Locale.CHINA, "%.02f", Float.valueOf(f4)) + " KB";
        }
        return String.format(Locale.CHINA, "%.02f", Float.valueOf(f4 / 1204.0f)) + " MB";
    }
    ...
    public static synchronized void process(String str, BitmapDesiredOptions bitmapDesiredOptions, boolean z3, BitmapCallback bitmapCallback) {
        synchronized (BitmapProcessor.class) {
            process(str, bitmapDesiredOptions, z3, true, bitmapCallback);
        }
    }

    public static synchronized void process(String str, BitmapDesiredOptions bitmapDesiredOptions, boolean z3, boolean z4, BitmapCallback bitmapCallback) {
        synchronized (BitmapProcessor.class) {
            process(str, bitmapDesiredOptions, z3, z4, 0L, bitmapCallback);
        }
    }

    public static synchronized void process(String str, BitmapDesiredOptions bitmapDesiredOptions, boolean z3, boolean z4, long j4, BitmapCallback bitmapCallback) {
        synchronized (BitmapProcessor.class) {
            if (str == null) {
                return;
            }
            synchronized (reqList) {
                ...
                for (int i2 = 0; i2 < size; i2++) {
                    ...
                    boolean z5 = (imageReq.bitmapDesiredOptions == null && bitmapDesiredOptions == null) || (imageReq.bitmapDesiredOptions != null && imageReq.bitmapDesiredOptions.equals(bitmapDesiredOptions));
                    if (equals && z5) {
                ...
                imageReq2.useRamCache = z3;
                imageReq2.diskCacheTime = j4;
                imageReq2.useDiskCache = z4;
