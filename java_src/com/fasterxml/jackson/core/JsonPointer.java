...
public class JsonPointer {
    ...
    private static void _appendEscape(StringBuilder sb, char c4) {
        if (c4 == '0') {
            c4 = '~';
        } else if (c4 == '1') {
            c4 = '/';
    ...
    protected static JsonPointer _parseQuotedTail(String str, int i2) {
        int length = str.length();
        ...
        while (i4 < length) {
            ...
            if (charAt == '/') {
                return new JsonPointer(str, sb.toString(), _parseTail(str.substring(i4)));
            }
            i4++;
            if (charAt == '~' && i4 < length) {
    ...
    public static JsonPointer compile(String str) throws IllegalArgumentException {
        if (str != null && str.length() != 0) {
            ...
            throw new IllegalArgumentException("Invalid input: JSON Pointer expression must start with '/': \"" + str + a.f72569g);
        }
        return EMPTY;
    }
    ...
    protected JsonPointer _constructHead(int i2, JsonPointer jsonPointer) {
        if (this == jsonPointer) {
            return EMPTY;
        }
