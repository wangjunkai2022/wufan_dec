        ...
        if (genericReturnType instanceof ParameterizedType) {
            ...
            if (Objects.equals(javaType.getRawClass(), parameterizedType.getRawType())) {
                ...
                for (int i2 = 0; i2 < actualTypeArguments.length; i2++) {
                    TypeVariable<?> maybeGetTypeVariable = maybeGetTypeVariable(actualTypeArguments[i2]);
                    if (maybeGetTypeVariable != null) {
                        String name = maybeGetTypeVariable.getName();
                        if (name == null || (boundType = javaType.getBindings().getBoundType(i2)) == null || (findByName = findByName(typeParameters, name)) == null) {
                            return null;
                        }
