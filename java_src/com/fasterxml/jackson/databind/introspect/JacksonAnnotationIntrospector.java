...
public class JacksonAnnotationIntrospector extends AnnotationIntrospector implements Serializable {
    ...
    public PropertyName findNameForDeserialization(Annotated annotated) {
        ...
        } else if (z3 || _hasOneOf(annotated, ANNOTATIONS_TO_INFER_DESER)) {
            return PropertyName.USE_DEFAULT;
        } else {
            return null;
        }
    ...
    public PropertyName findNameForSerialization(Annotated annotated) {
        ...
        if (jsonProperty != null) {
            ...
            return PropertyName.construct(jsonProperty.value(), str);
        } else if (z3 || _hasOneOf(annotated, ANNOTATIONS_TO_INFER_SER)) {
            return PropertyName.USE_DEFAULT;
        } else {
            return null;
        }
    ...
    public JavaType refineDeserializationType(MapperConfig<?> mapperConfig, Annotated annotated, JavaType javaType) throws JsonMappingException {
        ...
        if (_classIfExplicit != null && !javaType.hasRawClass(_classIfExplicit) && !_primitiveAndWrapper(javaType, _classIfExplicit)) {
            try {
                ...
                throw new JsonMappingException((Closeable) null, String.format("Failed to narrow type %s with annotation (value %s), from '%s': %s", javaType, _classIfExplicit.getName(), annotated.getName(), e4.getMessage()), e4);
            }
        }
        if (javaType.isMapLikeType()) {
            ...
            if (_classIfExplicit2 != null && !_primitiveAndWrapper(keyType, _classIfExplicit2)) {
                try {
                    ...
                    throw new JsonMappingException((Closeable) null, String.format("Failed to narrow key type of %s with concrete-type annotation (value %s), from '%s': %s", javaType, _classIfExplicit2.getName(), annotated.getName(), e5.getMessage()), e5);
                }
            }
        }
        ...
        if (contentType != null) {
            ...
            try {
                ...
                throw new JsonMappingException((Closeable) null, String.format("Failed to narrow value type of %s with concrete-type annotation (value %s), from '%s': %s", javaType, _classIfExplicit3.getName(), annotated.getName(), e6.getMessage()), e6);
            }
        }
        return javaType;
    }
        ...
        if (_classIfExplicit != null) {
            if (javaType.hasRawClass(_classIfExplicit)) {
                ...
                try {
                    ...
                    throw new JsonMappingException((Closeable) null, String.format("Failed to widen type %s with annotation (value %s), from '%s': %s", javaType, _classIfExplicit.getName(), annotated.getName(), e4.getMessage()), e4);
                }
            }
        }
        if (javaType.isMapLikeType()) {
            ...
            if (_classIfExplicit2 != null) {
                if (keyType.hasRawClass(_classIfExplicit2)) {
                    ...
                    try {
                        ...
                        throw new JsonMappingException((Closeable) null, String.format("Failed to widen key type of %s with concrete-type annotation (value %s), from '%s': %s", javaType, _classIfExplicit2.getName(), annotated.getName(), e5.getMessage()), e5);
                    }
                }
            ...
            if (_classIfExplicit3 != null) {
                if (contentType.hasRawClass(_classIfExplicit3)) {
                    ...
                    try {
                        ...
                        throw new JsonMappingException((Closeable) null, String.format("Internal error: failed to refine value type of %s with concrete-type annotation (value %s), from '%s': %s", javaType, _classIfExplicit3.getName(), annotated.getName(), e6.getMessage()), e6);
                    }
                }
