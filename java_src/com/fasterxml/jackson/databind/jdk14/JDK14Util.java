    ...
    static class RecordAccessor {
        ...
        public String[] getRecordFieldNames(Class<?> cls) throws IllegalArgumentException {
            ...
            for (int i2 = 0; i2 < recordComponents.length; i2++) {
                try {
                    strArr[i2] = (String) this.RECORD_COMPONENT_GET_NAME.invoke(recordComponents[i2], new Object[0]);
                } catch (Exception e4) {
                    throw new IllegalArgumentException(String.format("Failed to access name of field #%d (of %d) of Record type %s", Integer.valueOf(i2), Integer.valueOf(recordComponents.length), ClassUtil.nameOf(cls)), e4);
                }
            }
            return strArr;
        }
        ...
        public RawTypeName[] getRecordFields(Class<?> cls) throws IllegalArgumentException {
            ...
            for (int i2 = 0; i2 < recordComponents.length; i2++) {
                try {
                    try {
                        rawTypeNameArr[i2] = new RawTypeName((Class) this.RECORD_COMPONENT_GET_TYPE.invoke(recordComponents[i2], new Object[0]), (String) this.RECORD_COMPONENT_GET_NAME.invoke(recordComponents[i2], new Object[0]));
                    } catch (Exception e4) {
                        throw new IllegalArgumentException(String.format("Failed to access type of field #%d (of %d) of Record type %s", Integer.valueOf(i2), Integer.valueOf(recordComponents.length), ClassUtil.nameOf(cls)), e4);
                    }
                } catch (Exception e5) {
                    throw new IllegalArgumentException(String.format("Failed to access name of field #%d (of %d) of Record type %s", Integer.valueOf(i2), Integer.valueOf(recordComponents.length), ClassUtil.nameOf(cls)), e5);
                }
            }
