...
public abstract class BasicDeserializerFactory extends DeserializerFactory implements Serializable {
                ...
                while (true) {
                    if (i4 < parameterCount) {
                        ...
                        if (_findParamName != null && !_findParamName.isEmpty()) {
                            settableBeanPropertyArr2[i4] = constructCreatorProperty(deserializationContext, beanDescription, _findParamName, parameter.getIndex(), parameter, null);
                            i4++;
                        }
    ...
    protected void _addExplicitConstructorCreators(DeserializationContext deserializationContext, CreatorCollectionState creatorCollectionState, boolean z3) throws JsonMappingException {
        ...
        for (AnnotatedConstructor annotatedConstructor : beanDescription.getConstructors()) {
            ...
            if (JsonCreator.Mode.DISABLED != findCreatorAnnotation) {
                if (findCreatorAnnotation == null) {
                    if (z3 && visibilityChecker.isCreatorVisible(annotatedConstructor)) {
                    ...
                    int i2 = AnonymousClass1.$SwitchMap$com$fasterxml$jackson$annotation$JsonCreator$Mode[findCreatorAnnotation.ordinal()];
                    if (i2 == 1) {
    ...
    protected void _addExplicitDelegatingCreator(DeserializationContext deserializationContext, BeanDescription beanDescription, CreatorCollector creatorCollector, CreatorCandidate creatorCandidate) throws JsonMappingException {
        ...
        for (int i4 = 0; i4 < paramCount; i4++) {
            ...
            if (injection != null) {
                settableBeanPropertyArr[i4] = constructCreatorProperty(deserializationContext, beanDescription, null, i4, parameter, injection);
            } else if (i2 < 0) {
                i2 = i4;
            } else {
                deserializationContext.reportBadTypeDefinition(beanDescription, "More than one argument (#%d and #%d) left as delegating for Creator %s: only one allowed", Integer.valueOf(i2), Integer.valueOf(i4), creatorCandidate);
            }
        }
        if (i2 < 0) {
    ...
    protected void _addExplicitFactoryCreators(DeserializationContext deserializationContext, CreatorCollectionState creatorCollectionState, boolean z3) throws JsonMappingException {
        BeanDescription beanDescription = creatorCollectionState.beanDesc;
        CreatorCollector creatorCollector = creatorCollectionState.creators;
        AnnotationIntrospector annotationIntrospector = creatorCollectionState.annotationIntrospector();
        VisibilityChecker<?> visibilityChecker = creatorCollectionState.vchecker;
        ...
        for (AnnotatedMethod annotatedMethod : beanDescription.getFactoryMethods()) {
            ...
            if (findCreatorAnnotation == null) {
                if (z3 && parameterCount == 1 && visibilityChecker.isCreatorVisible(annotatedMethod)) {
                ...
                if (i8 > 0 || i9 > 0) {
                    if (i12 + i9 == i11) {
                        ...
                        if (findImplicitParamName == null || findImplicitParamName.isEmpty()) {
                            deserializationContext.reportBadTypeDefinition(beanDescription, "Argument #%d of constructor %s has no property name annotation; must have name when multiple-parameter constructor annotated as Creator", Integer.valueOf(i10), annotatedWithParams2);
                        }
                    }
                }
    ...
    protected void _addRecordConstructor(DeserializationContext deserializationContext, CreatorCollectionState creatorCollectionState, AnnotatedConstructor annotatedConstructor, List<String> list) throws JsonMappingException {
        ...
        for (int i2 = 0; i2 < parameterCount; i2++) {
            ...
            if (findNameForDeserialization == null || findNameForDeserialization.isEmpty()) {
                findNameForDeserialization = PropertyName.construct(list.get(i2));
            }
            settableBeanPropertyArr[i2] = constructCreatorProperty(deserializationContext, creatorCollectionState.beanDesc, findNameForDeserialization, i2, parameter, findInjectableValue);
        }
    ...
    protected boolean _handleSingleArgumentCreator(CreatorCollector creatorCollector, AnnotatedWithParams annotatedWithParams, boolean z3, boolean z4) {
        Class<?> rawParameterType = annotatedWithParams.getRawParameterType(0);
        if (rawParameterType == String.class || rawParameterType == CLASS_CHAR_SEQUENCE) {
            if (z3 || z4) {
                creatorCollector.addStringCreator(annotatedWithParams, z3);
            }
            return true;
        } else if (rawParameterType == Integer.TYPE || rawParameterType == Integer.class) {
            if (z3 || z4) {
                creatorCollector.addIntCreator(annotatedWithParams, z3);
            }
            return true;
        } else if (rawParameterType == Long.TYPE || rawParameterType == Long.class) {
            if (z3 || z4) {
                creatorCollector.addLongCreator(annotatedWithParams, z3);
            }
            return true;
        } else if (rawParameterType == Double.TYPE || rawParameterType == Double.class) {
            if (z3 || z4) {
                creatorCollector.addDoubleCreator(annotatedWithParams, z3);
            }
            return true;
        } else if (rawParameterType == Boolean.TYPE || rawParameterType == Boolean.class) {
            if (z3 || z4) {
                creatorCollector.addBooleanCreator(annotatedWithParams, z3);
            }
            return true;
        } else {
            if (rawParameterType == BigInteger.class && (z3 || z4)) {
                creatorCollector.addBigIntegerCreator(annotatedWithParams, z3);
            }
            if (rawParameterType == BigDecimal.class && (z3 || z4)) {
                creatorCollector.addBigDecimalCreator(annotatedWithParams, z3);
            }
            if (z3) {
                creatorCollector.addDelegatingCreator(annotatedWithParams, z3, null, 0);
                return true;
            }
            return false;
        }
    ...
    protected SettableBeanProperty constructCreatorProperty(DeserializationContext deserializationContext, BeanDescription beanDescription, PropertyName propertyName, int i2, AnnotatedParameter annotatedParameter, JacksonInject.Value value) throws JsonMappingException {
        ...
        CreatorProperty construct2 = CreatorProperty.construct(propertyName, resolveMemberAndTypeAnnotations, std.getWrapperName(), typeDeserializer, beanDescription.getClassAnnotations(), annotatedParameter, i2, value, _getSetterInfo(deserializationContext, std, propertyMetadata));
    ...
    */
    ...
    public TypeDeserializer findPropertyTypeDeserializer(DeserializationConfig deserializationConfig, JavaType javaType, AnnotatedMember annotatedMember) throws JsonMappingException {
        ...
        try {
            ...
            InvalidDefinitionException from = InvalidDefinitionException.from((JsonParser) null, ClassUtil.exceptionMessage(e4), javaType);
            from.initCause(e4);
            throw from;
        }
    }
    ...
    protected void _addExplicitAnyCreator(DeserializationContext deserializationContext, BeanDescription beanDescription, CreatorCollector creatorCollector, CreatorCandidate creatorCandidate, ConstructorDetector constructorDetector) throws JsonMappingException {
        ...
        int i2 = AnonymousClass1.$SwitchMap$com$fasterxml$jackson$databind$cfg$ConstructorDetector$SingleArgConstructor[constructorDetector.singleArgMode().ordinal()];
        if (i2 == 1) {
            propertyName = null;
            z3 = false;
        } else if (i2 == 2) {
            ...
            z3 = (explicitParamName == null && injection == null) ? false : true;
