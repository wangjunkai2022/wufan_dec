...
public abstract class BaseDaoImpl<T, ID> implements Dao<T, ID> {
    ...
    public CloseableIterator<T> createIterator(int i2) {
        try {
            ...
            throw new IllegalStateException("Could not build iterator for " + this.dataClass, e4);
        }
    }

    private <FT> ForeignCollection<FT> makeEmptyForeignCollection(T t3, String str) throws SQLException {
        FieldType[] fieldTypes;
        checkForInitialized();
        ID extractId = t3 == null ? null : extractId(t3);
        for (FieldType fieldType : this.tableInfo.getFieldTypes()) {
            if (fieldType.getColumnName().equals(str)) {
                BaseForeignCollection buildForeignCollection = fieldType.buildForeignCollection(t3, extractId);
                if (t3 != null) {
                    fieldType.assignField(t3, buildForeignCollection, true, null);
                }
    ...
    public void assignEmptyForeignCollection(T t3, String str) throws SQLException {
        makeEmptyForeignCollection(t3, str);
    }
    ...
    @Override // com.j256.ormlite.dao.Dao
    public int create(T t3) throws SQLException {
        checkForInitialized();
        if (t3 == null) {
            return 0;
        }
        if (t3 instanceof BaseDaoEnabled) {
            ((BaseDaoEnabled) t3).setDao(this);
        }
        DatabaseConnection readWriteConnection = this.connectionSource.getReadWriteConnection();
        try {
            return this.statementExecutor.create(readWriteConnection, t3, this.objectCache);
        } finally {
            this.connectionSource.releaseConnection(readWriteConnection);
        }
    }
    ...
    @Override // com.j256.ormlite.dao.Dao
    public T createIfNotExists(T t3) throws SQLException {
        if (t3 == null) {
            return null;
        }
        T queryForSameId = queryForSameId(t3);
    ...
    @Override // com.j256.ormlite.dao.Dao
    public Dao.CreateOrUpdateStatus createOrUpdate(T t3) throws SQLException {
        if (t3 == null) {
            return new Dao.CreateOrUpdateStatus(false, false, 0);
        }
        ID extractId = extractId(t3);
        if (extractId != null && idExists(extractId)) {
            return new Dao.CreateOrUpdateStatus(false, true, update((BaseDaoImpl<T, ID>) t3));
        }
        return new Dao.CreateOrUpdateStatus(true, false, create(t3));
    }

    @Override // com.j256.ormlite.dao.Dao
    public int delete(T t3) throws SQLException {
        checkForInitialized();
        if (t3 == null) {
            return 0;
        }
        DatabaseConnection readWriteConnection = this.connectionSource.getReadWriteConnection();
        try {
            return this.statementExecutor.delete(readWriteConnection, t3, this.objectCache);
        } finally {
            this.connectionSource.releaseConnection(readWriteConnection);
        }
    }
    ...
    public int executeRaw(String str, String... strArr) throws SQLException {
        ...
        try {
            try {
                ...
                throw SqlExceptionUtil.create("Could not run raw execute statement " + str, e4);
            }
        } finally {
            this.connectionSource.releaseConnection(readWriteConnection);
        }
    ...
    @Override // com.j256.ormlite.dao.Dao
    public ID extractId(T t3) throws SQLException {
        checkForInitialized();
        FieldType idField = this.tableInfo.getIdField();
        if (idField != null) {
            return (ID) idField.extractJavaFieldValue(t3);
        }
        ...
        return new CloseableWrappedIterableImpl(new CloseableIterable<T>() { // from class: com.j256.ormlite.dao.BaseDaoImpl.2
            ...
            public CloseableIterator<T> closeableIterator() {
                try {
                    ...
                    throw new IllegalStateException("Could not build iterator for " + BaseDaoImpl.this.dataClass, e4);
                }
            }
    ...
    public String objectToString(T t3) {
        checkForInitialized();
        return this.tableInfo.objectToString(t3);
    }

    @Override // com.j256.ormlite.dao.Dao
    public boolean objectsEqual(T t3, T t4) throws SQLException {
        FieldType[] fieldTypes;
        checkForInitialized();
        for (FieldType fieldType : this.tableInfo.getFieldTypes()) {
            if (!fieldType.getDataPersister().dataIsEqual(fieldType.extractJavaFieldValue(t3), fieldType.extractJavaFieldValue(t4))) {
                return false;
            }
        }
        return true;
    ...
    @Override // com.j256.ormlite.dao.Dao
    public List<T> queryForMatching(T t3) throws SQLException {
        return queryForMatching(t3, false);
    }

    @Override // com.j256.ormlite.dao.Dao
    public List<T> queryForMatchingArgs(T t3) throws SQLException {
        return queryForMatching(t3, true);
    }

    @Override // com.j256.ormlite.dao.Dao
    public T queryForSameId(T t3) throws SQLException {
        ID extractId;
        checkForInitialized();
        if (t3 == null || (extractId = extractId(t3)) == null) {
            return null;
        }
    ...
    public GenericRawResults<String[]> queryRaw(String str, String... strArr) throws SQLException {
        ...
        try {
            ...
            throw SqlExceptionUtil.create("Could not perform raw query for " + str, e4);
        }
    }
    ...
    public long queryRawValue(String str, String... strArr) throws SQLException {
        ...
        try {
            try {
                ...
                throw SqlExceptionUtil.create("Could not perform raw value query for " + str, e4);
            }
        } finally {
            this.connectionSource.releaseConnection(readOnlyConnection);
        }
    ...
    public int refresh(T t3) throws SQLException {
        ...
        DatabaseConnection readOnlyConnection = this.connectionSource.getReadOnlyConnection();
        try {
            return this.statementExecutor.refresh(readOnlyConnection, t3, this.objectCache);
        } finally {
            this.connectionSource.releaseConnection(readOnlyConnection);
        }
    }
    ...
    @Override // com.j256.ormlite.dao.Dao
    public void setAutoCommit(boolean z3) throws SQLException {
        DatabaseConnection readWriteConnection = this.connectionSource.getReadWriteConnection();
        try {
            setAutoCommit(readWriteConnection, z3);
        } finally {
            this.connectionSource.releaseConnection(readWriteConnection);
        }
    }
    ...
    @Override // com.j256.ormlite.dao.Dao
    public void setObjectCache(boolean z3) throws SQLException {
    ...
    public int update(T t3) throws SQLException {
        ...
        DatabaseConnection readWriteConnection = this.connectionSource.getReadWriteConnection();
        try {
            return this.statementExecutor.update(readWriteConnection, t3, this.objectCache);
        } finally {
            this.connectionSource.releaseConnection(readWriteConnection);
        }
    }
    ...
    @Override // com.j256.ormlite.dao.Dao
    public int updateId(T t3, ID id) throws SQLException {
        checkForInitialized();
        if (t3 == null) {
            return 0;
        }
        DatabaseConnection readWriteConnection = this.connectionSource.getReadWriteConnection();
        try {
            return this.statementExecutor.updateId(readWriteConnection, t3, id, this.objectCache);
        } finally {
            this.connectionSource.releaseConnection(readWriteConnection);
        }
    }
    ...
    public int updateRaw(String str, String... strArr) throws SQLException {
        ...
        try {
            try {
                ...
                throw SqlExceptionUtil.create("Could not run raw update statement " + str, e4);
            }
        } finally {
            this.connectionSource.releaseConnection(readWriteConnection);
        }
    ...
    private List<T> queryForFieldValues(Map<String, Object> map, boolean z3) throws SQLException {
        checkForInitialized();
    ...
    private List<T> queryForMatching(T t3, boolean z3) throws SQLException {
        FieldType[] fieldTypes;
        checkForInitialized();
        QueryBuilder<T, ID> queryBuilder = queryBuilder();
        Where<T, ID> where = queryBuilder.where();
        int i2 = 0;
        for (FieldType fieldType : this.tableInfo.getFieldTypes()) {
            Object fieldValueIfNotDefault = fieldType.getFieldValueIfNotDefault(t3);
    ...
    public CloseableIterator<T> createIterator(PreparedQuery<T> preparedQuery, int i2) throws SQLException {
        try {
            ...
            throw SqlExceptionUtil.create("Could not build prepared-query iterator for " + this.dataClass, e4);
        }
    }
        ...
        return new CloseableWrappedIterableImpl(new CloseableIterable<T>() { // from class: com.j256.ormlite.dao.BaseDaoImpl.3
            ...
            public CloseableIterator<T> closeableIterator() {
                try {
                    ...
                    throw new IllegalStateException("Could not build prepared-query iterator for " + BaseDaoImpl.this.dataClass, e4);
                }
            }
    ...
    @Override // com.j256.ormlite.dao.Dao
    public void setAutoCommit(DatabaseConnection databaseConnection, boolean z3) throws SQLException {
        databaseConnection.setAutoCommit(z3);
    }
