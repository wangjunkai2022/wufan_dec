...
public class TableInfo<T, ID> {
    ...
    private static <T, ID> void wireNewInstance(BaseDaoImpl<T, ID> baseDaoImpl, T t3) {
        if (t3 instanceof BaseDaoEnabled) {
            ((BaseDaoEnabled) t3).setDao(baseDaoImpl);
        }
    }
    ...
    public T createObject() throws SQLException {
        ...
        try {
            ...
            throw SqlExceptionUtil.create("Could not create object for " + this.constructor.getDeclaringClass(), e4);
        }
    }
    ...
    public String objectToString(T t3) {
        ...
        for (FieldType fieldType : this.fieldTypes) {
            ...
            try {
                sb.append(fieldType.extractJavaFieldValue(t3));
            } catch (Exception e4) {
    ...
    public TableInfo(DatabaseType databaseType, BaseDaoImpl<T, ID> baseDaoImpl, DatabaseTableConfig<T> databaseTableConfig) throws SQLException {
        ...
        for (FieldType fieldType2 : fieldTypes) {
            ...
            z3 = fieldType2.isForeignAutoCreate() ? true : z3;
