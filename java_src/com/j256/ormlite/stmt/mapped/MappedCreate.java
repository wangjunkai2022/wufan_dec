...
public class MappedCreate<T, ID> extends BaseMappedStatement<T, ID> {
    ...
    private void assignIdValue(T t3, Number number, String str, ObjectCache objectCache) throws SQLException {
        this.idField.assignIdValue(t3, number, objectCache);
    ...
    private void assignSequenceId(DatabaseConnection databaseConnection, T t3, ObjectCache objectCache) throws SQLException {
        ...
        if (queryForLong != 0) {
            assignIdValue(t3, Long.valueOf(queryForLong), "sequence", objectCache);
            return;
        }
    ...
    public static <T, ID> MappedCreate<T, ID> build(DatabaseType databaseType, TableInfo<T, ID> tableInfo) {
        ...
        return new MappedCreate<>(tableInfo, sb.toString(), fieldTypeArr, buildQueryNextSequence(databaseType, tableInfo.getIdField()), i4);
    }
