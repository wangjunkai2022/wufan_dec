...
public class MappedUpdateId<T, ID> extends BaseMappedStatement<T, ID> {
    ...
    private Object extractIdToFieldObject(T t3) throws SQLException {
        return this.idField.extractJavaFieldToSqlArgValue(t3);
    }

    /* JADX WARN: Multi-variable type inference failed */
    public int execute(DatabaseConnection databaseConnection, T t3, ID id, ObjectCache objectCache) throws SQLException {
        Object updateId;
        try {
            Object[] objArr = {convertIdToFieldObject(id), extractIdToFieldObject(t3)};
            int update = databaseConnection.update(this.statement, objArr, this.argFieldTypes);
            if (update > 0) {
                if (objectCache != 0 && (updateId = objectCache.updateId(this.clazz, this.idField.extractJavaFieldValue(t3), id)) != null && updateId != t3) {
                    this.idField.assignField(updateId, id, false, objectCache);
                }
                this.idField.assignField(t3, id, false, objectCache);
            }
            ...
            throw SqlExceptionUtil.create("Unable to run update-id stmt on object " + t3 + ": " + this.statement, e4);
        }
    }
}
