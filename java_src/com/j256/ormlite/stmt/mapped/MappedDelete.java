...
public class MappedDelete<T, ID> extends BaseMappedStatement<T, ID> {
    ...
    public int delete(DatabaseConnection databaseConnection, T t3, ObjectCache objectCache) throws SQLException {
        try {
            ...
            if (delete > 0 && objectCache != 0) {
                objectCache.remove(this.clazz, this.idField.extractJavaFieldToSqlArgValue(t3));
            }
            return delete;
        } catch (SQLException e4) {
            throw SqlExceptionUtil.create("Unable to run delete stmt on object " + t3 + ": " + this.statement, e4);
        }
    }
    ...
    public int deleteById(DatabaseConnection databaseConnection, ID id, ObjectCache objectCache) throws SQLException {
        try {
            ...
            throw SqlExceptionUtil.create("Unable to run deleteById stmt on id " + id + ": " + this.statement, e4);
        }
    }
}
