...
public class MappedRefresh<T, ID> extends MappedQueryForId<T, ID> {
    ...
    public int executeRefresh(DatabaseConnection databaseConnection, T t3, ObjectCache objectCache) throws SQLException {
        FieldType[] fieldTypeArr;
        Object execute = super.execute(databaseConnection, this.idField.extractJavaFieldValue(t3), null);
        if (execute == null) {
            return 0;
        }
        for (FieldType fieldType : this.resultsFieldTypes) {
            if (fieldType != this.idField) {
                fieldType.assignField(t3, fieldType.extractJavaFieldValue(execute), false, objectCache);
            }
        }
        return 1;
    }
