...
public class QueryBuilder<T, ID> extends StatementBuilder<T, ID> {
        ...
        if (list != null) {
            for (QueryBuilder<T, ID>.JoinInfo joinInfo : list) {
                ...
                if (queryBuilder != null && queryBuilder.hasGroupStuff()) {
                    joinInfo.queryBuilder.appendGroupBys(sb, z3);
                }
            }
        }
    }
        ...
        if (list2 != null) {
            for (QueryBuilder<T, ID>.JoinInfo joinInfo : list2) {
                ...
                if (queryBuilder != null && queryBuilder.hasOrderStuff()) {
                    joinInfo.queryBuilder.appendOrderBys(sb, z3, list);
                }
            }
        }
    }
    ...
    private void setAddTableName(boolean z3) {
        this.addTableName = z3;
        List<QueryBuilder<T, ID>.JoinInfo> list = this.joinList;
        if (list != null) {
            for (QueryBuilder<T, ID>.JoinInfo joinInfo : list) {
                joinInfo.queryBuilder.setAddTableName(z3);
            }
        }
    }
    ...
    public boolean appendWhereStatement(StringBuilder sb, List<ArgumentHolder> list, StatementBuilder.WhereOperation whereOperation) throws SQLException {
        ...
        if (list2 != null) {
            for (QueryBuilder<T, ID>.JoinInfo joinInfo : list2) {
                ...
                z3 = joinInfo.queryBuilder.appendWhereStatement(sb, list, whereOperation2);
            }
        }
        return z3;
    }
    ...
    public QueryBuilder<T, ID> orderBy(String str, boolean z3) {
        if (!verifyColumnName(str).isForeignCollection()) {
            ...
            this.orderByList.add(new OrderBy(str, z3));
            return this;
        }
    ...
    private void appendGroupBys(StringBuilder sb, boolean z3) {
        if (z3) {
            sb.append("GROUP BY ");
        }
    ...
    private void appendOrderBys(StringBuilder sb, boolean z3, List<ArgumentHolder> list) {
        if (z3) {
            sb.append("ORDER BY ");
        }
