...
public class WebAuthorizeActivity extends AbstractAuthorizeActivity implements Handler.Callback, ResizeLayout.OnResizeListener {
    ...
    public void OnResize(int i2, int i4, int i5, int i6) {
        AuthorizeAdapter authorizeAdapter = this.adapter;
        if (authorizeAdapter != null) {
            authorizeAdapter.onResize(i2, i4, i5, i6);
        }
    }
        ...
        new Thread() { // from class: cn.sharesdk.framework.authorize.WebAuthorizeActivity.2
            ...
            public void run() {
                try {
                    ...
                    message.what = 2;
                    if ("none".equals(DeviceHelper.getInstance(((FakeActivity) WebAuthorizeActivity.this).activity).getDetailNetworkTypeForStatic())) {
                        message.arg1 = 1;
    ...
    public void onCreate() {
        if (this.rv == null) {
            ...
            TitleLayout c4 = this.rv.c();
            this.adapter.setTitleView(c4);
            String name = this.helper.getPlatform().getName();
            this.adapter.setPlatformName(this.helper.getPlatform().getName());
            try {
                Context context = getContext();
                c4.getTvTitle().setText(ResHelper.getStringRes(context, "ssdk_" + name.toLowerCase()));
            } catch (Throwable th) {
                try {
                    c4.getTvTitle().setText(ResHelper.getStringRes(getContext(), "ssdk_weibo_oauth_regiseter"));
    ...
    public boolean onKeyEvent(int i2, KeyEvent keyEvent) {
        ...
        if (!onKeyEvent && i2 == 4 && keyEvent.getAction() == 0 && (authorizeListener = this.helper.getAuthorizeListener()) != null) {
            authorizeListener.onCancel();
        }
        if (onKeyEvent) {
            return true;
