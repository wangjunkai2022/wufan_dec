...
public abstract class AbstractCoalescingBufferQueue {
    ...
    public AbstractCoalescingBufferQueue(Channel channel, int i2) {
        this.bufAndListenerPairs = new ArrayDeque<>(i2);
        this.tracker = channel == null ? null : PendingBytesTracker.newTracker(channel);
    }

    private void decrementReadableBytes(int i2) {
        this.readableBytes -= i2;
        PendingBytesTracker pendingBytesTracker = this.tracker;
        if (pendingBytesTracker != null) {
            pendingBytesTracker.decrementPendingOutboundBytes(i2);
        }
    }

    private void incrementReadableBytes(int i2) {
        int i4 = this.readableBytes;
        int i5 = i4 + i2;
        if (i5 >= i4) {
            this.readableBytes = i5;
            PendingBytesTracker pendingBytesTracker = this.tracker;
            if (pendingBytesTracker != null) {
                pendingBytesTracker.incrementPendingOutboundBytes(i2);
                return;
            }
            return;
        }
        throw new IllegalStateException("buffer queue length overflow: " + this.readableBytes + " + " + i2);
    }
