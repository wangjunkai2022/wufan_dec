...
public final class KQueueEventLoop extends SingleThreadEventLoop {
    ...
    private int kqueueWait(boolean z3) throws IOException {
        ...
        int min = (int) Math.min(delayNanos / k.f56834a, 2147483647L);
        return kqueueWait(min, (int) Math.min(delayNanos - (min * k.f56834a), 2147483647L));
    }
    ...
    private void processReady(int i2) {
        for (int i4 = 0; i4 < i2; i4++) {
            short filter = this.eventList.filter(i4);
            short flags = this.eventList.flags(i4);
            int fd = this.eventList.fd(i4);
            if (filter != Native.EVFILT_USER && (Native.EV_ERROR & flags) == 0) {
                ...
                if (abstractKQueueChannel == null) {
                    logger.warn("events[{}]=[{}, {}] had no channel!", Integer.valueOf(i4), Integer.valueOf(this.eventList.fd(i4)), Short.valueOf(filter));
                } else {
                    ...
                    if (filter == Native.EVFILT_WRITE) {
                        ...
                        abstractKQueueUnsafe.readReady(this.eventList.data(i4));
                    } else if (filter == Native.EVFILT_SOCK && (this.eventList.fflags(i4) & Native.NOTE_RDHUP) != 0) {
    ...
    public void evSet(AbstractKQueueChannel abstractKQueueChannel, short s3, short s4, int i2) {
        this.changeList.evSet(abstractKQueueChannel, s3, s4, i2);
    }
    ...
    */
    ...
    private int kqueueWait(int i2, int i4) throws IOException {
        int keventWait = Native.keventWait(this.kqueueFd.intValue(), this.changeList, this.eventList, i2, i4);
