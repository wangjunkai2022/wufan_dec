...
public final class EpollDatagramChannel extends AbstractEpollChannel implements DatagramChannel {
            ...
            try {
                do {
                    ...
                    if (Native.IS_SUPPORTING_RECVMMSG) {
                        i2 = maxDatagramPayloadSize == 0 ? 1 : allocate.writableBytes() / maxDatagramPayloadSize;
                    } else {
                        i2 = 0;
                    }
                    if (i2 > 1) {
                        scatteringRead = EpollDatagramChannel.this.scatteringRead(recvBufAllocHandle, allocate, maxDatagramPayloadSize, i2);
                    } else if (isConnected) {
                        try {
                            scatteringRead = EpollDatagramChannel.this.connectedRead(recvBufAllocHandle, allocate, maxDatagramPayloadSize);
                        } catch (Errors.NativeIoException e4) {
                            if (isConnected) {
                                throw EpollDatagramChannel.this.translateForConnected(e4);
                            }
                            throw e4;
                        }
                    } else {
    ...
    public boolean connectedRead(EpollRecvByteAllocatorHandle epollRecvByteAllocatorHandle, ByteBuf byteBuf, int i2) throws Exception {
        ...
        try {
            if (i2 != 0) {
                writableBytes = Math.min(byteBuf.writableBytes(), i2);
            } else {
    ...
    */
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public boolean read(EpollRecvByteAllocatorHandle epollRecvByteAllocatorHandle, ByteBuf byteBuf, int i2) throws IOException {
        int writableBytes;
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public boolean scatteringRead(EpollRecvByteAllocatorHandle epollRecvByteAllocatorHandle, ByteBuf byteBuf, int i2, int i4) throws IOException {
        RecyclableArrayList recyclableArrayList = null;
        try {
            int writerIndex = byteBuf.writerIndex();
            NativeDatagramPacketArray cleanDatagramPacketArray = cleanDatagramPacketArray();
            int i5 = 0;
            while (i5 < i4 && cleanDatagramPacketArray.addWritable(byteBuf, writerIndex, i2)) {
                i5++;
                writerIndex += i2;
            }
            ...
            for (int i7 = 0; i7 < recvmmsg; i7++) {
                newInstance.add(packets[i7].newDatagramPacket(byteBuf.readRetainedSlice(i2), localAddress));
            }
            epollRecvByteAllocatorHandle.lastBytesRead(i6);
            epollRecvByteAllocatorHandle.incMessagesRead(recvmmsg);
            for (int i8 = 0; i8 < recvmmsg; i8++) {
                pipeline().fireChannelRead(newInstance.set(i8, Unpooled.EMPTY_BUFFER));
            }
            newInstance.recycle();
            byteBuf.release();
            return true;
            ...
            if (0 != 0) {
                for (int i9 = 0; i9 < recyclableArrayList.size(); i9++) {
                    ReferenceCountUtil.release(recyclableArrayList.get(i9));
                }
        ...
        while (true) {
            ...
            if (Native.IS_SUPPORTING_SENDMMSG && channelOutboundBuffer.size() > 1) {
                ...
                if (count >= 1) {
                    ...
                    int i2 = 0;
                    while (count > 0) {
                        int sendmmsg = this.socket.sendmmsg(packets, i2, count);
                        if (sendmmsg == 0) {
    ...
    */
    ...
    private EpollDatagramChannel(LinuxSocket linuxSocket, boolean z3) {
        super((Channel) null, linuxSocket, z3);
