...
public final class CompositeNameResolver<T> extends SimpleNameResolver<T> {
    ...
    public CompositeNameResolver(EventExecutor eventExecutor, NameResolver<T>... nameResolverArr) {
        ...
        for (int i2 = 0; i2 < nameResolverArr.length; i2++) {
            NameResolver<T> nameResolver = nameResolverArr[i2];
            ObjectUtil.checkNotNull(nameResolver, "resolvers[" + i2 + ']');
        }
    ...
    public void doResolveAllRec(final String str, final Promise<List<T>> promise, final int i2, Throwable th) throws Exception {
        NameResolver<T>[] nameResolverArr = this.resolvers;
        if (i2 >= nameResolverArr.length) {
            promise.setFailure(th);
        } else {
            nameResolverArr[i2].resolveAll(str).addListener(new a<List<T>>() { // from class: io.netty.resolver.CompositeNameResolver.2
                @Override // io.netty.util.concurrent.GenericFutureListener
                public void operationComplete(Future<List<T>> future) throws Exception {
                    if (!future.isSuccess()) {
                        CompositeNameResolver.this.doResolveAllRec(str, promise, i2 + 1, future.cause());
                    } else {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public void doResolveRec(final String str, final Promise<T> promise, final int i2, Throwable th) throws Exception {
        NameResolver<T>[] nameResolverArr = this.resolvers;
        if (i2 >= nameResolverArr.length) {
            promise.setFailure(th);
        } else {
            nameResolverArr[i2].resolve(str).addListener(new a<T>() { // from class: io.netty.resolver.CompositeNameResolver.1
                @Override // io.netty.util.concurrent.GenericFutureListener
                public void operationComplete(Future<T> future) throws Exception {
                    if (!future.isSuccess()) {
                        CompositeNameResolver.this.doResolveRec(str, promise, i2 + 1, future.cause());
                    } else {
                        promise.setSuccess(future.getNow());
                    }
                }
