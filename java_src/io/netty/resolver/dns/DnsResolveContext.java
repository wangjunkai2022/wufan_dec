...
public abstract class DnsResolveContext<T> {
    ...
    public static final class AuthoritativeNameServerList {
        ...
        void add(DnsRecord dnsRecord) {
            ...
            if ((authoritativeNameServer != null && authoritativeNameServer.dots > i2) || (decodeDomainName = DnsResolveContext.decodeDomainName(((ByteBufHolder) dnsRecord).content())) == null) {
                return;
            }
            AuthoritativeNameServer authoritativeNameServer2 = this.head;
            if (authoritativeNameServer2 != null && authoritativeNameServer2.dots >= i2) {
                if (this.head.dots != i2) {
                    return;
                }
                ...
                while (true) {
                    ...
                    if (authoritativeNameServer4 == null) {
                        authoritativeNameServer3.next = new AuthoritativeNameServer(i2, dnsRecord.timeToLive(), lowerCase, decodeDomainName);
                ...
                this.head = new AuthoritativeNameServer(i2, dnsRecord.timeToLive(), lowerCase, decodeDomainName);
            }
        }
            ...
            while (authoritativeNameServer != null) {
                if (authoritativeNameServer.address == null) {
                    ...
                    if (list != null && !list.isEmpty() && (address = list.get(0).address()) != null) {
                        ...
                        int i2 = 1;
                        while (i2 < list.size()) {
                            InetAddress address2 = list.get(i2).address();
        ...
        public Iterator<InetSocketAddress> iterator() {
            return new Iterator<InetSocketAddress>() { // from class: io.netty.resolver.dns.DnsResolveContext.DnsAddressStreamList.1
                ...
                public boolean hasNext() {
                    return this.f65773i < this.stream.size();
                }
    ...
    private static String hostnameWithDot(String str) {
        if (StringUtil.endsWith(str, d.f65364a)) {
            return str;
        }
    ...
    public void internalResolve(String str, Promise<List<T>> promise) {
        ...
        for (int i2 = 0; i2 < length; i2++) {
            if (!query(str, this.expectedTypes[i2], nameServers.duplicate(), false, promise)) {
                return;
            }
        }
    ...
    */
    ...
    public void onResponse(DnsServerAddressStream dnsServerAddressStream, int i2, DnsQuestion dnsQuestion, AddressedEnvelope<DnsResponse, InetSocketAddress> addressedEnvelope, DnsQueryLifecycleObserver dnsQueryLifecycleObserver, Promise<List<T>> promise) {
        try {
            ...
            if (code != DnsResponseCode.NXDOMAIN) {
                query(dnsServerAddressStream, i2 + 1, dnsQuestion, dnsQueryLifecycleObserver.queryNoAnswer(code), true, promise, null);
            } else {
                dnsQueryLifecycleObserver.queryFailed(NXDOMAIN_QUERY_FAILED_EXCEPTION);
                if (!content.isAuthoritativeAnswer()) {
                    query(dnsServerAddressStream, i2 + 1, dnsQuestion, newDnsQueryLifecycleObserver(dnsQuestion), true, promise, null);
                }
            }
        } finally {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public void query(final DnsServerAddressStream dnsServerAddressStream, final int i2, final DnsQuestion dnsQuestion, final DnsQueryLifecycleObserver dnsQueryLifecycleObserver, boolean z3, final Promise<List<T>> promise, Throwable th) {
        if (!this.completeEarly && i2 < dnsServerAddressStream.size() && this.allowedQueries != 0 && !promise.isCancelled()) {
            this.allowedQueries--;
            InetSocketAddress next = dnsServerAddressStream.next();
            if (next.isUnresolved()) {
                queryUnresolvedNameserver(next, dnsServerAddressStream, i2, dnsQuestion, dnsQueryLifecycleObserver, promise, th);
                return;
            }
            ...
            query0.addListener(new a<AddressedEnvelope<DnsResponse, InetSocketAddress>>() { // from class: io.netty.resolver.dns.DnsResolveContext.2
                ...
                public void operationComplete(Future<AddressedEnvelope<DnsResponse, InetSocketAddress>> future) {
                    ...
                    try {
                        if (cause == null) {
                            DnsResolveContext.this.onResponse(dnsServerAddressStream, i2, dnsQuestion, future.getNow(), dnsQueryLifecycleObserver, promise);
                        } else {
                            ...
                            dnsResolveContext.query(dnsServerAddressStream, i2 + 1, dnsQuestion2, dnsResolveContext.newDnsQueryLifecycleObserver(dnsQuestion2), true, promise, cause);
                        }
                    } finally {
                        DnsResolveContext.this.tryToFinishResolve(dnsServerAddressStream, i2, dnsQuestion, NoopDnsQueryLifecycleObserver.INSTANCE, promise, cause);
                    }
                }
            });
            return;
        }
        tryToFinishResolve(dnsServerAddressStream, i2, dnsQuestion, dnsQueryLifecycleObserver, promise, th);
    }

    private void queryUnresolvedNameserver(final InetSocketAddress inetSocketAddress, final DnsServerAddressStream dnsServerAddressStream, final int i2, final DnsQuestion dnsQuestion, final DnsQueryLifecycleObserver dnsQueryLifecycleObserver, final Promise<List<T>> promise, final Throwable th) {
        ...
        newPromise.addListener((GenericFutureListener<? extends Future<? super List<T>>>) new a<List<InetAddress>>() { // from class: io.netty.resolver.dns.DnsResolveContext.3
            ...
            public void operationComplete(Future<List<InetAddress>> future) {
                ...
                if (!future.isSuccess()) {
                    DnsResolveContext.this.query(dnsServerAddressStream, i2 + 1, dnsQuestion, dnsQueryLifecycleObserver, true, promise, th);
                    return;
                }
                DnsResolveContext.this.query(new CombinedDnsServerAddressStream(inetSocketAddress, future.getNow(), dnsServerAddressStream), i2, dnsQuestion, dnsQueryLifecycleObserver, true, promise, th);
            }
        });
        ...
        if (this.finalResult == null) {
            if (i2 < dnsServerAddressStream.size()) {
                if (dnsQueryLifecycleObserver == NoopDnsQueryLifecycleObserver.INSTANCE) {
                    query(dnsServerAddressStream, 1 + i2, dnsQuestion, newDnsQueryLifecycleObserver(dnsQuestion), true, promise, th);
                    return;
                } else {
                    query(dnsServerAddressStream, 1 + i2, dnsQuestion, dnsQueryLifecycleObserver, true, promise, th);
                    return;
                }
            }
    ...
    abstract DnsResolveContext<T> newResolverContext(DnsNameResolver dnsNameResolver, String str, int i2, DnsRecordType[] dnsRecordTypeArr, DnsRecord[] dnsRecordArr, DnsServerAddressStream dnsServerAddressStream);

    /* JADX INFO: Access modifiers changed from: package-private */
    public void resolve(Promise<List<T>> promise) {
        String str;
        String[] searchDomains = this.parent.searchDomains();
        if (searchDomains.length != 0 && this.parent.ndots() != 0 && !StringUtil.endsWith(this.hostname, d.f65364a)) {
            boolean hasNDots = hasNDots();
            if (hasNDots) {
                str = this.hostname;
            } else {
                str = this.hostname + d.f65364a + searchDomains[0];
            }
            int i2 = !hasNDots ? 1 : 0;
            Promise<List<T>> newPromise = this.parent.executor().newPromise();
            newPromise.addListener((GenericFutureListener<? extends Future<? super List<T>>>) new a<List<T>>(i2, promise, searchDomains, hasNDots) { // from class: io.netty.resolver.dns.DnsResolveContext.1
                ...
                public void operationComplete(Future<List<T>> future) {
                    ...
                    if (cause == null) {
                        ...
                        int i4 = this.searchDomainIdx;
                        this.searchDomainIdx = i4 + 1;
                        sb.append(strArr[i4]);
    ...
    public static final class AuthoritativeNameServer {
        ...
        AuthoritativeNameServer(int i2, long j4, String str, String str2) {
            this.dots = i2;
            this.ttl = j4;
            this.nsName = str2;
            this.domainName = str;
