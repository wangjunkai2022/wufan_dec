...
public final class PemX509Certificate extends X509Certificate implements PemEncoded {
    ...
    private static ByteBuf append(ByteBufAllocator byteBufAllocator, boolean z3, PemEncoded pemEncoded, int i2, ByteBuf byteBuf) {
        ByteBuf content = pemEncoded.content();
        if (byteBuf == null) {
            byteBuf = newBuffer(byteBufAllocator, z3, content.readableBytes() * i2);
        }
        byteBuf.writeBytes(content.slice());
        return byteBuf;
    }

    private static ByteBuf newBuffer(ByteBufAllocator byteBufAllocator, boolean z3, int i2) {
        return z3 ? byteBufAllocator.directBuffer(i2) : byteBufAllocator.buffer(i2);
    }
    ...
    private static ByteBuf append(ByteBufAllocator byteBufAllocator, boolean z3, X509Certificate x509Certificate, int i2, ByteBuf byteBuf) throws CertificateEncodingException {
        ...
        try {
            ...
            if (byteBuf == null) {
                byteBuf = newBuffer(byteBufAllocator, z3, (BEGIN_CERT.length + base64.readableBytes() + END_CERT.length) * i2);
            }
