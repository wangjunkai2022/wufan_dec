...
public class Socks5PasswordAuthRequestDecoder extends ReplayingDecoder<State> {
    ...
    protected void decode(ChannelHandlerContext channelHandlerContext, ByteBuf byteBuf, List<Object> list) throws Exception {
        try {
            int i2 = AnonymousClass1.$SwitchMap$io$netty$handler$codec$socksx$v5$Socks5PasswordAuthRequestDecoder$State[state().ordinal()];
            if (i2 == 1) {
                int readerIndex = byteBuf.readerIndex();
                byte b4 = byteBuf.getByte(readerIndex);
                if (b4 == 1) {
                    short unsignedByte = byteBuf.getUnsignedByte(readerIndex + 1);
                    int i4 = readerIndex + 2;
                    short unsignedByte2 = byteBuf.getUnsignedByte(i4 + unsignedByte);
                    byteBuf.skipBytes(unsignedByte + unsignedByte2 + 3);
                    Charset charset = CharsetUtil.US_ASCII;
                    list.add(new DefaultSocks5PasswordAuthRequest(byteBuf.toString(i4, unsignedByte, charset), byteBuf.toString(readerIndex + 3 + unsignedByte, unsignedByte2, charset)));
