...
public class Socks5ServerEncoder extends MessageToByteEncoder<a> {
    ...
    public void encode(ChannelHandlerContext channelHandlerContext, a aVar, ByteBuf byteBuf) throws Exception {
        if (aVar instanceof Socks5InitialResponse) {
            encodeAuthMethodResponse((Socks5InitialResponse) aVar, byteBuf);
        } else if (aVar instanceof Socks5PasswordAuthResponse) {
            encodePasswordAuthResponse((Socks5PasswordAuthResponse) aVar, byteBuf);
        } else if (aVar instanceof Socks5CommandResponse) {
            encodeCommandResponse((Socks5CommandResponse) aVar, byteBuf);
        } else {
