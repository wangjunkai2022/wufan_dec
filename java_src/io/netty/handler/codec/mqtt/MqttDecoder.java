...
public final class MqttDecoder extends ReplayingDecoder<DecoderState> {
    ...
    private static Result<MqttConnectPayload> decodeConnectionPayload(ByteBuf byteBuf, MqttConnectVariableHeader mqttConnectVariableHeader) {
        ...
        if (MqttCodecUtil.isValidClientId(MqttVersion.fromProtocolNameAndLevel(mqttConnectVariableHeader.name(), (byte) mqttConnectVariableHeader.version()), str)) {
            ...
            if (mqttConnectVariableHeader.isWillFlag()) {
                ...
                int i4 = i2 + ((Result) result).numberOfBytesConsumed;
                result2 = decodeByteArray(byteBuf);
                i2 = i4 + ((Result) result2).numberOfBytesConsumed;
            } else {
                result = null;
                result2 = null;
            }
            ...
            return new Result<>(new MqttConnectPayload((String) ((Result) decodeString).value, result != null ? (String) ((Result) result).value : null, result2 != null ? (byte[]) ((Result) result2).value : null, result3 != null ? (String) ((Result) result3).value : null, result4 != null ? (byte[]) ((Result) result4).value : null), i2);
        }
    ...
    private static Result<MqttConnectVariableHeader> decodeConnectionVariableHeader(ByteBuf byteBuf) {
        ...
        Result<Integer> decodeMsbLsb = decodeMsbLsb(byteBuf);
        int i4 = i2 + 1 + 1 + ((Result) decodeMsbLsb).numberOfBytesConsumed;
        ...
        return new Result<>(new MqttConnectVariableHeader(fromProtocolNameAndLevel.protocolName(), fromProtocolNameAndLevel.protocolLevel(), z3, z4, z5, i5, z6, z7, ((Integer) ((Result) decodeMsbLsb).value).intValue()), i4);
    }
    ...
    private static MqttFixedHeader decodeFixedHeader(ByteBuf byteBuf) {
        ...
        while (true) {
            ...
            i2 = ((readUnsignedByte2 & 127) * i8) + i5;
            i8 *= 128;
            i7++;
        ...
        return MqttCodecUtil.validateFixedHeader(MqttCodecUtil.resetUnusedFields(new MqttFixedHeader(valueOf, z3, MqttQoS.valueOf(i6), z4, i2)));
    }
    ...
    private static Result<?> decodePayload(ByteBuf byteBuf, MqttMessageType mqttMessageType, int i2, Object obj) {
        int i4 = AnonymousClass1.$SwitchMap$io$netty$handler$codec$mqtt$MqttMessageType[mqttMessageType.ordinal()];
        if (i4 != 1) {
            if (i4 != 11) {
                if (i4 != 3) {
                    if (i4 != 4) {
                        ...
                        return decodeSubackPayload(byteBuf, i2);
                    }
                    return decodeUnsubscribePayload(byteBuf, i2);
                }
                return decodeSubscribePayload(byteBuf, i2);
            }
            return decodePublishPayload(byteBuf, i2);
        }
    ...
    private static Result<MqttPublishVariableHeader> decodePublishVariableHeader(ByteBuf byteBuf, MqttFixedHeader mqttFixedHeader) {
        ...
        if (MqttCodecUtil.isValidPublishTopicName((String) ((Result) decodeString).value)) {
            ...
            if (mqttFixedHeader.qosLevel().value() > 0) {
                Result<Integer> decodeMessageId = decodeMessageId(byteBuf);
                i4 = ((Integer) ((Result) decodeMessageId).value).intValue();
                i2 += ((Result) decodeMessageId).numberOfBytesConsumed;
            }
            return new Result<>(new MqttPublishVariableHeader((String) ((Result) decodeString).value, i4), i2);
        }
    ...
    private static Result<MqttSubscribePayload> decodeSubscribePayload(ByteBuf byteBuf, int i2) {
        ...
        while (i4 < i2) {
            Result<String> decodeString = decodeString(byteBuf);
            i4 = i4 + ((Result) decodeString).numberOfBytesConsumed + 1;
    ...
    */
    ...
    private static Result<Integer> decodeMsbLsb(ByteBuf byteBuf, int i2, int i4) {
        int readUnsignedByte = byteBuf.readUnsignedByte() | (byteBuf.readUnsignedByte() << 8);
        return new Result<>(Integer.valueOf((readUnsignedByte < i2 || readUnsignedByte > i4) ? -1 : -1), 2);
    }

    private static Result<String> decodeString(ByteBuf byteBuf, int i2, int i4) {
        Result<Integer> decodeMsbLsb = decodeMsbLsb(byteBuf);
        int intValue = ((Integer) ((Result) decodeMsbLsb).value).intValue();
        int i5 = ((Result) decodeMsbLsb).numberOfBytesConsumed;
        if (intValue >= i2 && intValue <= i4) {
            ...
            return new Result<>(byteBuf2, i5 + intValue);
        }
        byteBuf.skipBytes(intValue);
        return new Result<>(null, i5 + intValue);
    }
}
