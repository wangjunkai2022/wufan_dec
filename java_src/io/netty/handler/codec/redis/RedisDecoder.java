...
public final class RedisDecoder extends ByteToMessageDecoder {
    ...
    private boolean decodeLength(ByteBuf byteBuf, List<Object> list) throws Exception {
        ...
        if (parseRedisNumber >= -1) {
            int i2 = AnonymousClass1.$SwitchMap$io$netty$handler$codec$redis$RedisMessageType[this.type.ordinal()];
            if (i2 == 1) {
    ...
    private a newInlineRedisMessage(RedisMessageType redisMessageType, ByteBuf byteBuf) {
        int i2 = AnonymousClass1.$SwitchMap$io$netty$handler$codec$redis$RedisMessageType[redisMessageType.ordinal()];
        if (i2 != 3) {
            if (i2 == 4) {
    ...
    private long parseRedisNumber(ByteBuf byteBuf) {
        int readableBytes = byteBuf.readableBytes();
        int i2 = (readableBytes <= 0 || byteBuf.getByte(byteBuf.readerIndex()) != 45) ? 0 : 1;
        if (readableBytes <= i2) {
            throw new RedisCodecException("no number to parse: " + byteBuf.toString(CharsetUtil.US_ASCII));
        } else if (readableBytes <= i2 + 19) {
            if (i2 != 0) {
                return -parsePositiveNumber(byteBuf.skipBytes(i2));
            }
    ...
    */
    ...
    public RedisDecoder(boolean z3) {
        this(65536, FixedRedisMessagePool.INSTANCE, z3);
    }
    ...
    public RedisDecoder(int i2, RedisMessagePool redisMessagePool, boolean z3) {
