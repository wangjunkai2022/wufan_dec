...
public class Http2ConnectionHandler extends ByteToMessageDecoder implements Http2LifecycleManager, ChannelOutboundHandler {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public static void processGoAwayWriteResult(ChannelHandlerContext channelHandlerContext, int i2, long j4, ByteBuf byteBuf, ChannelFuture channelFuture) {
        try {
            if (channelFuture.isSuccess()) {
                if (j4 != Http2Error.NO_ERROR.code()) {
                    InternalLogger internalLogger = logger;
                    if (internalLogger.isDebugEnabled()) {
                        internalLogger.debug("{} Sent GOAWAY: lastStreamId '{}', errorCode '{}', debugData '{}'. Forcing shutdown of the connection.", channelHandlerContext.channel(), Integer.valueOf(i2), Long.valueOf(j4), byteBuf.toString(CharsetUtil.UTF_8), channelFuture.cause());
                    }
                ...
                if (internalLogger2.isDebugEnabled()) {
                    internalLogger2.debug("{} Sending GOAWAY failed: lastStreamId '{}', errorCode '{}', debugData '{}'. Forcing shutdown of the connection.", channelHandlerContext.channel(), Integer.valueOf(i2), Long.valueOf(j4), byteBuf.toString(CharsetUtil.UTF_8), channelFuture.cause());
                }
                channelHandlerContext.close();
            }
        } finally {
    ...
    private ChannelFuture resetUnknownStream(final ChannelHandlerContext channelHandlerContext, int i2, long j4, ChannelPromise channelPromise) {
        ChannelFuture writeRstStream = frameWriter().writeRstStream(channelHandlerContext, i2, j4, channelPromise);
    ...
    public void closeStreamLocal(Http2Stream http2Stream, ChannelFuture channelFuture) {
        int i2 = AnonymousClass6.$SwitchMap$io$netty$handler$codec$http2$Http2Stream$State[http2Stream.state().ordinal()];
        if (i2 != 1 && i2 != 2) {
            closeStream(http2Stream, channelFuture);
        } else {
    ...
    public void flush(ChannelHandlerContext channelHandlerContext) {
        try {
            ...
            onError(channelHandlerContext, true, e4);
        } catch (Throwable th) {
    ...
    @Override // io.netty.handler.codec.http2.Http2LifecycleManager
    public ChannelFuture goAway(final ChannelHandlerContext channelHandlerContext, final int i2, final long j4, final ByteBuf byteBuf, ChannelPromise channelPromise) {
        ChannelPromise unvoid = channelPromise.unvoid();
        try {
            if (!connection().goAwaySent(i2, j4, byteBuf)) {
                byteBuf.release();
                unvoid.trySuccess();
                return unvoid;
            }
            byteBuf.retain();
            ChannelFuture writeGoAway = frameWriter().writeGoAway(channelHandlerContext, i2, j4, byteBuf, unvoid);
            if (writeGoAway.isDone()) {
                processGoAwayWriteResult(channelHandlerContext, i2, j4, byteBuf, writeGoAway);
            } else {
                writeGoAway.addListener((GenericFutureListener<? extends Future<? super Void>>) new ChannelFutureListener() { // from class: io.netty.handler.codec.http2.Http2ConnectionHandler.5
                    @Override // io.netty.util.concurrent.GenericFutureListener
                    public void operationComplete(ChannelFuture channelFuture) throws Exception {
                        Http2ConnectionHandler.processGoAwayWriteResult(channelHandlerContext, i2, j4, byteBuf, channelFuture);
                    }
                });
            }
            return writeGoAway;
    ...
    @Override // io.netty.handler.codec.http2.Http2LifecycleManager
    public void onError(ChannelHandlerContext channelHandlerContext, boolean z3, Throwable th) {
        ...
        if (Http2Exception.isStreamError(embeddedHttp2Exception)) {
            onStreamError(channelHandlerContext, z3, th, (Http2Exception.StreamException) embeddedHttp2Exception);
            ...
            while (it2.hasNext()) {
                onStreamError(channelHandlerContext, z3, th, it2.next());
            }
        } else {
            onConnectionError(channelHandlerContext, z3, th, embeddedHttp2Exception);
        }
        channelHandlerContext.flush();
    }
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public void onStreamError(ChannelHandlerContext channelHandlerContext, boolean z3, Throwable th, Http2Exception.StreamException streamException) {
        ...
        if ((streamException instanceof Http2Exception.HeaderListSizeException) && ((Http2Exception.HeaderListSizeException) streamException).duringDecode() && connection().isServer()) {
            ...
            if (stream != null && !stream.isHeadersSent()) {
                try {
                    ...
                    onError(channelHandlerContext, z3, Http2Exception.connectionError(Http2Error.INTERNAL_ERROR, th2, "Error DecodeSizeError", new Object[0]));
                }
            }
        }
        Http2Stream http2Stream = stream;
        if (http2Stream == null) {
            if (!z3 || connection().local().mayHaveCreatedStream(streamId)) {
                resetUnknownStream(channelHandlerContext, streamId, streamException.error().code(), channelHandlerContext.newPromise());
                return;
            }
            return;
    ...
    @Override // io.netty.handler.codec.http2.Http2LifecycleManager
    public ChannelFuture resetStream(ChannelHandlerContext channelHandlerContext, int i2, long j4, ChannelPromise channelPromise) {
        Http2Stream stream = connection().stream(i2);
        if (stream == null) {
            return resetUnknownStream(channelHandlerContext, i2, j4, channelPromise.unvoid());
        }
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public Http2ConnectionHandler(Http2ConnectionDecoder http2ConnectionDecoder, Http2ConnectionEncoder http2ConnectionEncoder, Http2Settings http2Settings, boolean z3) {
        ...
        this.decoupleCloseAndGoAway = z3;
